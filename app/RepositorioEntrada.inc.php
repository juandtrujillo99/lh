...
class RepositorioEntrada {
    ...
    public static function insertar_entrada($conexion, $entrada) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "INSERT INTO entradas(autor_id, url, video, imagen, imagen2, titulo, precio, texto, fecha, activa) VALUES(:autor_id, :url, :video, :imagen, :imagen2, :titulo, :precio, :texto, NOW(), :activa)";
                ...
                $sentencia -> bindParam(':video', $entrada -> obtener_video(), PDO::PARAM_STR);
                $sentencia -> bindParam(':imagen', $entrada -> obtener_imagen(), PDO::PARAM_STR);
                $sentencia -> bindParam(':imagen2', $entrada -> obtener_imagen2(), PDO::PARAM_STR);
    ...
    public static function obtener_todas_por_fecha_descendiente($conexion) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM entradas ORDER BY fecha ASC LIMIT 1000";//aqui se modifica el numero de entradas que deben aparecer en una pagina especifica
                ...
                if (count($resultado)) {
                    foreach ($resultado as $fila) {
                        ...
                        $entradas[] = new Entrada(
                                ...
                                $fila['video'], $fila['imagen'], $fila['imagen2'], $fila['titulo'],
                                $fila['precio'], $fila['texto'], $fila['fecha'], $fila['activa']
                        );
    ...
    public static function obtener_cuatro_por_fecha_descendiente($conexion) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM entradas ORDER BY fecha DESC LIMIT 8";//aqui se modifica el numero de entradas que deben aparecer en una pagina especifica
    ...
    public static function obtener_entrada_por_url($conexion, $url) {
        ...
        if (isset($conexion)) {
            try {
                ...
                if (!empty($resultado)) {
                    $entrada = new Entrada(
                            $resultado['id'], $resultado['autor_id'], $resultado['url'], $resultado['video'], $resultado['imagen'], $resultado['imagen2'], $resultado['titulo'],
                                $resultado['precio'], $resultado['texto'], $resultado['fecha'], $resultado['activa']
                            );
                }
            } catch (PDOException $ex) {
                print 'ERROR'. $ex -> getMessage();
    	...
    	if (isset($conexion)) {
    		try {
    			...
    			if (count($resultado)) {
    				foreach ($resultado as $fila) {
    					$entradas[] = new Entrada(
    						$fila['id'], $fila['autor_id'], $fila['url'], $fila['video'], $fila['imagen'], $fila['imagen2'], $fila['titulo'],
                                $fila['precio'], $fila['texto'], $fila['fecha'], $fila['activa']
    					);
    				}
    			}
    ...
    public static function obtener_entradas_usuario_fecha_descendente($conexion, $id_usuario) {
    	...
    	if (isset($conexion)) {
    		try {
    			$sql = "SELECT a.id, a.autor_id, a.url, a.video, a.imagen, a.imagen2, a.titulo, a.precio, a.texto, a.fecha, a.activa, COUNT(b.id) AS 'cantidad_comentarios' ";
    			$sql .= "FROM entradas a ";
    ...
    public static function obtener_entradas_usuario_titulo_descendente($conexion, $id_usuario) {
        $entradas_obtenidas = [];
    ...
    public static function actualizar_entrada($conexion, $id, $titulo, $url, $video, $imagen, $imagen2, $precio, $texto, $activa) {
        $actualizacion_correcta = false;

        if (isset($conexion)) {
            try {
                $sql = "UPDATE entradas SET titulo = :titulo, url = :url, video = :video, imagen = :imagen, imagen2 = :imagen2, precio = :precio, texto = :texto, activa = :activa WHERE id = :id";
                ...
                $sentencia -> bindParam(':imagen2', $imagen2, PDO::PARAM_STR);
